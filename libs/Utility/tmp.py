...
REPO_URL = ''
REPO_REV = 'master'
URL = 'ssh://%s@gerrit.sensethink.cn:29418/git-repo'
...
def _Init(args, gitc_init=False):
    ...
    try:
        ...
        if can_verify and opt.no_repo_verify:
...
def main(orig_args):
    ...
    # _print("wqj args len: %d" % len(args))
    if len(args) > 1 and "//" in args[1] and "@" in args[1]:
        username = args[1].split("//")[1].split("@")[0]
        REPO_URL = URL % username

    # _print("wqj url--->%s" % REPO_URL)
